...
import time
import threading
answer_list= {0:["命令注入","9","flag{test}"],1:["栈溢出","35","flag{9f794cb8-23d7-4164-80d9-56a14bd27b5b}"],2:["格式化字符串","26","flag{test}"],3:["SQL注入","21","flag{1af05f25-1beb-46a2-a9eb-998de9c90e2f}"],4:["文件包含","9","flag{b8df00c0-8bd4-4e60-84bc-3d3fbe8b3d63}"]}
url_list = ["http://127.0.0.1:8000"]
file_list = ["http://150.158.100.181:60000/code_inject_demo.c","http://150.158.100.181:60000/stackoverflow_demo.c","http://150.158.100.181:60000/format_demo.c","http://150.158.100.181:60000/sql_inject.php","http://150.158.100.181:60000/index.php"]
chall_url = ["150.158.100.181:50000","150.158.100.181:50003","150.158.100.181:50006","150.158.100.181:50009","150.158.100.181:50012"]
...
def question4(index,url):
    try:
        params = {"message":"目标地址为{0},请尝试进行漏洞利用。".format(chall_url[index])}
        response = requests.get(url+"/chat",params=params,timeout=10)
        text = response.text
        print(text)
...
def main():
    for i in range(1):
        thread = threading.Thread(target=competition,args=(i,url_list[i],))
